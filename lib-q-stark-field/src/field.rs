    ...
    + DeserializeOwned
{
    ...
    ///
    /// # Security
    /// This method uses standard equality comparison which may not be constant-time.
    /// For constant-time comparison of secret values, use `constant_time::constant_time_is_zero()`
    /// which requires the field to implement `ConstantTimeEq`.
    #[must_use]
    #[inline]
    ...
    /// # Security
    /// This method uses standard equality comparison which may not be constant-time.
    /// For constant-time comparison of secret values, use constant-time operations from
    /// the `constant_time` module which require the field to implement `ConstantTimeEq`.
    #[must_use]
    #[inline]
    fn is_one(&self) -> bool {
        *self == Self::ONE
