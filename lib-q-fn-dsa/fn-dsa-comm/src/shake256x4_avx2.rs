...
pub(crate) unsafe fn init_seed(sh: &mut SHAKE256x4, seed: &[u8]) {
    // SAFETY: the caller of this `unsafe fn` guarantees AVX2 is available;
    // edition 2024 requires the unsafe operations to be spelled out.
    unsafe {
        let mut i = 0;
        let mut jh = 0;
        let mut jl = 0;
...
pub(crate) unsafe fn refill(sh: &mut SHAKE256x4) {
    ...
    unsafe {
        ...
        // `as_mut_ptr`, not `as_ptr`: `d` is written through below, so it must carry
        // write provenance. The previous form took a *const from a `&mut` borrow and
        // transmuted the constness away, which is exactly the pattern that makes a
        // pointer's provenance not cover the writes performed with it.
        let d: *mut __m256i = core::mem::transmute(sh.buf.as_mut_ptr());
        for i in 0..17 {
