...
use lib_q_stark_field::PackedValue;
use lib_q_stark_matrix::{
    Dimensions,
    Matrix,
};
...
mod tests {
    ...
    fn commit_single_1x8() {
        ...
        let mmcs = MyMmcs::new(hash, compress.clone());
    ...
    fn verify_tampered_proof_fails() {
        ...
        let dims = large_mat_dims.chain(small_mat_dims).collect_vec();
        let (commit, prover_data) = mmcs.commit(mats);

        // Test tampering at various positions in the proof
        for index in [0, 3, 7] {
            let batch_opening = mmcs.open_batch(index, &prover_data);
            let proof_len = batch_opening.opening_proof.len();

            // Test tampering at each level of the proof
            for proof_level in 0..proof_len {
                // Test tampering at each element in the digest array
                for element_idx in 0..8 {
                    let mut tampered_opening = batch_opening.clone();
                    let original_value = tampered_opening.opening_proof[proof_level][element_idx];
                    tampered_opening.opening_proof[proof_level][element_idx] += F::ONE;

                    assert_ne!(
                        tampered_opening.opening_proof[proof_level][element_idx], original_value,
                        "proof modification should change the value at level {proof_level}, element {element_idx}"
                    );

                    let result =
                        mmcs.verify_batch(&commit, &dims, index, (&tampered_opening).into());
                    result.expect_err(
                        "expected verification to fail on tampered proof at level {proof_level}, element {element_idx}"
                    );
                }
            }
        }

        // Test tampering of opened values
        let batch_opening = mmcs.open_batch(3, &prover_data);
        for matrix_idx in 0..batch_opening.opened_values.len() {
            if !batch_opening.opened_values[matrix_idx].is_empty() {
                let mut tampered_opening = batch_opening.clone();
                let original_first = tampered_opening.opened_values[matrix_idx][0];
                tampered_opening.opened_values[matrix_idx][0] += F::ONE;

                assert_ne!(
                    tampered_opening.opened_values[matrix_idx][0], original_first,
                    "opened value modification should change the value for matrix {matrix_idx}"
                );

                let result = mmcs.verify_batch(&commit, &dims, 3, (&tampered_opening).into());
                result.expect_err(
                    "expected verification to fail on tampered opened value for matrix {matrix_idx}"
                );
            }
        }
    }
