...
// p3_blake3::Blake3 removed: non-NIST hash
...
// p3_rescue::Rescue removed: non-NIST hash
...
fn bench_merkle_trees(criterion: &mut Criterion) {
    ...
    // bench_bb_rescue(criterion); // Rescue removed: non-NIST hash
    // bench_bb_blake3(criterion); // Blake3 removed: non-NIST hash
...
fn bench_bb_poseidon2(criterion: &mut Criterion) {
    ...
    bench_mmcs::<<F as Field>::Packing, <F as Field>::Packing, H, C, 8>(criterion, h, c.clone());
...
fn bench_bb_keccak(criterion: &mut Criterion) {
    ...
    type H = SerializingHasher<Shake256Hash>;
    let k = Shake256Hash {};
    let h = H::new(k);

    type C = CompressionFunctionFromHasher<Shake256Hash, 2, 32>;
    let c = C::new(k);
