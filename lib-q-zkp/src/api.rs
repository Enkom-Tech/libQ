...
///
/// This verifies a proof generated by [`prove_preimage`]. The verifier reconstructs the public
/// Poseidon commitment from the **preimage**, so the caller passes the same secret bytes given
/// to [`prove_preimage`] (NOT the hash output).
///
/// # Arguments
///
/// * `proof` - The proof to verify
/// * `preimage` - The secret preimage (same bytes passed to [`prove_preimage`])
///
/// # Returns
///
/// `Ok(true)` if the proof is valid, `Ok(false)` or `Err` otherwise
pub fn verify_preimage(proof: &ZkpProof, preimage: &[u8]) -> Result<bool> {
    let verifier = ZkpVerifier::new();
    verifier.verify_secret_value(proof, preimage)
}
...
mod tests {
    ...
    #[test]
    fn test_preimage_nist_api_returns_not_implemented() {
        // The NIST AIR has no Keccak-f constraints yet, so the public API must refuse to
        // prove or verify rather than emit/accept an unsound proof.
        let secret = b"api-preimage-secret-nist";
        assert!(matches!(
            prove_preimage_nist(secret),
        ...
        assert!(matches!(
            verify_preimage_nist(&dummy, &[0u8; 32]),
            Err(lib_q_core::Error::NotImplemented { .. })
        ));
    }
}
