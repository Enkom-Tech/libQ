...
fn test_it_ownership_correct_it_verifies() {
    ...
    assert!(
        verify_it_ownership(&proof, &it1).unwrap_or(false),
        "correct IT (from identity_token_from_secret) must verify"
    );
}
...
fn test_it_ownership_wrong_key_same_it_fails() {
    ...
    assert!(
        !verify_it_ownership(&proof, &it).unwrap_or(false),
        "proof with wrong key (same IT) must not verify"
    );
}
...
fn test_it_ownership_wrong_it_fails() {
    ...
    assert!(
        !verify_it_ownership(&proof, &it2).unwrap_or(false),
        "wrong IT must not verify"
    );
}
...
fn test_group_membership_wrong_root_fails() {
    ...
    assert!(
        !verify_group_membership(&proof, &wrong_root).unwrap_or(false),
...
fn test_group_membership_cross_tree_fails() {
    ...
    assert!(
        !verify_group_membership(&proof_t1, &root_t2).unwrap_or(false),
        "proof for tree 1 must not verify against root of tree 2"
    );
}
...
fn test_credential_correct_disclosure_verifies() {
    ...
    // Verification requires expected_commitment (commitment bytes); the credential
    // API does not expose commitment derivation. We assert prove succeeds and
    // wrong disclosed value fails in test_credential_wrong_disclosed_value_fails.
    assert!(!proof.data.is_empty());
    let _ = (disclosed_val_100, proof);
}
...
fn test_credential_wrong_disclosed_value_fails() {
    ...
    assert!(
        !verify_credential_proof(&proof, &[], &wrong_val_101).unwrap_or(false),
        "wrong disclosed value must not verify"
    );
}
