//! Tests for the new modular architecture without existing lib.rs dependencies

// Only import the new modules we created
...
fn test_key_validation() {
    ...
    let result = validate_kem_key(HpkeKem::MlKem512, &kem_key, false);
    if let Err(e) = &result {
        println!("KEM key validation error: {:?}", e);
    }
    assert!(result.is_ok());
