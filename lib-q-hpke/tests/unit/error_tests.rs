...
fn test_kdf_error_creation() {
    let error = HpkeError::kdf_error(
        HpkeKdf::HkdfShake256,
        KdfOperation::Extract,
        "Test error",
    );
    
    match error {
        HpkeError::KdfError { algorithm, operation, cause } => {
...
fn test_aead_error_creation() {
    let error = HpkeError::aead_error(
    ...
    match error {
        HpkeError::AeadError { algorithm, operation, cause } => {
...
fn test_security_error_creation() {
    let error = HpkeError::security_error(
...
fn test_protocol_error_creation() {
    let error = HpkeError::protocol_error(
        ProtocolStage::KeySchedule,
        "Test error",
    );
...
fn test_invalid_input_error_creation() {
    ...
    match error {
        HpkeError::InvalidInput { parameter, value, expected } => {
