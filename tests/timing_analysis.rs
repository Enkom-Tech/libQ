...
fn test_hash_timing_consistency() {
    ...
    assert!(max_time.as_nanos() <= min_time.as_nanos() * 10, 
...
fn test_key_comparison_timing() {
    ...
    assert!(max_time.as_nanos() <= min_time.as_nanos() * 5, 
...
fn test_random_generation_timing() {
    ...
    if time3.as_nanos() > 0 && time1.as_nanos() > 0 {
        ...
        assert!(ratio > 0.1, "Random generation timing seems completely random: ratio={ratio}");
    }
}
...
fn test_utility_function_timing() {
    ...
    assert!(max_time.as_nanos() <= min_time.as_nanos() * 20, 
...
fn test_validation_timing() {
    ...
    assert!(max_time.as_nanos() <= min_time.as_nanos() * 50, 
...
fn test_no_timing_patterns() {
    ...
    let even_timings: Vec<Duration> = timings.iter().enumerate()
        .filter(|(i, _)| i % 2 == 0)
        .map(|(_, &t)| t)
        .collect();
    
    let odd_timings: Vec<Duration> = timings.iter().enumerate()
        .filter(|(i, _)| i % 2 == 1)
        .map(|(_, &t)| t)
        .collect();
    ...
    assert!(max_avg.as_nanos() <= min_avg.as_nanos() * 3, 
