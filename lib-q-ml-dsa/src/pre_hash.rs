...
use crate::constants::CONTEXT_MAX_LEN;
use crate::{
    SigningError,
    VerificationError,
    hash_functions,
...
pub struct DomainSeparationContext<'a> {
    context: &'a [u8],
    pre_hash_oid: Option<PreHashOID>,
}

pub enum DomainSeparationError {
    ContextTooLongError,
}
...
impl<'a> DomainSeparationContext<'a> {
    ...
    pub fn new(context: &'a [u8], pre_hash_oid: Option<PreHashOID>) -> PreHashResult<'a> {
