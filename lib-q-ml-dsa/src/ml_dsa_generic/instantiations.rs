// SUPERSEDED 2026-08-10. The comment here previously recorded that all 6 functions generated by
// `parameter_set!` below were dead in every configuration, because `ml_dsa_44.rs` etc. called
// `crate::ml_dsa_generic::ml_dsa_44::*` directly and bypassed this macro's output. That was
// accurate when written and is no longer true: the public `generate_key_pair`/`sign`/`verify`
// now go through `ml_dsa_generic::multiplexing`, whose portable fallback calls
// `instantiations::portable::<set>::{generate_key_pair,sign,verify}` unconditionally (the
// avx2/neon arms are runtime-guarded, so the portable arm is compiled in on every target).
// Those three are therefore live and no longer carry `#[allow(dead_code)]`.
//
// Still dead, and still carrying the attribute: `sign_mut`, `sign_pre_hashed_shake128` and
// `verify_pre_hashed_shake128`. The pre-hashed HashML-DSA entry points in `ml_dsa_44.rs` etc.
// still hardcode portable rather than dispatching, so those two remain an unwired path; wiring
// them is separate work from the sign/verify/keygen fix and was deliberately left alone.
