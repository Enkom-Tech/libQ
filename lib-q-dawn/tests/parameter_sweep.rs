...
/// Phase 1 deep diagnostic: actual c2 error count distribution (raw values, not just ≤7 buckets).
/// Reports min/p1/p5/p25/median/p75/p95/p99/max for each parameter point.
/// Run: cargo test -p lib-q-dawn --features random --release --test parameter_sweep test_phase1_c2_error_depth -- --ignored --nocapture
#[test]
#[ignore = "Phase 1 deep c2 error diagnostic; run with --ignored --nocapture"]
fn test_phase1_c2_error_depth() {
    use lib_q_dawn::encoding::pke_c2_error_diagnostic;
    use rand_core::TryRng;

    let grid: &[(usize, usize, u32)] = &[
        (16, 24, 1),
        (16, 24, 2),
        (16, 32, 1),
        (24, 32, 1),
    ...
    eprintln!("profile,ks,ke,dc,min,p1,p5,p25,median,p75,p95,p99,max,mean,cprime_abs_mean_avg");
    for &(k_s, k_e, d_c) in grid {
        let params = KeyGenParams::dawn_alpha_512_custom(k_s, k_e, d_c);
        let keypair = keypair_for_alpha512_dc_grid(k_s, k_e, d_c);

        let encoder = lib_q_dawn::encoding::DoubleEncoder::new(
            params.degree,
        ...
        let n = params.degree;
        let m_len = n / 4 / 8;
        let ks_half = params.s_coeff_count / 2;
        let ke_half = params.e_coeff_count / 2;

        let mut rng_seed = [0u8; 64];
        rng_seed.copy_from_slice(
            &lib_q_dawn::security::generate_deterministic_high_entropy_data(
                &[
                    b"phase1_c2_depth".as_ref(),
                    &(k_s as u64).to_le_bytes(),
                    &(k_e as u64).to_le_bytes(),
                    &d_c.to_le_bytes(),
                ]
        ...
        let mut rng = lib_q_dawn::keygen::DawnRng::new_deterministic(&rng_seed);

        let mut distances = Vec::with_capacity(pke_n);
        let mut cprime_abs_mean_sum = 0.0f64;

        for _ in 0..pke_n {
            let mut message = vec![0u8; m_len];
            let _ = TryRng::try_fill_bytes(&mut rng, &mut message);
            let s = lib_q_dawn::polynomial::field::FieldPolynomial::random_ternary_exact(
                n,
                ks_half,
                params.large_modulus,
                &mut rng,
            );
            let e = lib_q_dawn::polynomial::field::FieldPolynomial::random_ternary_exact(
                n,
                ke_half,
                params.large_modulus,
                &mut rng,
            );
            let ct = match lib_q_dawn::encoding::pke_encrypt(
                &keypair.public_key,
                &message,
                &s,
                &e,
            ...
            let diag = pke_c2_error_diagnostic(&ct, &keypair.secret_key, &message, &encoder);
            distances.push(diag.c2_hamming_distance);
            cprime_abs_mean_sum += diag.c_prime_abs_mean;
        }

        distances.sort();
        let cnt = distances.len();
        if cnt == 0 {
            eprintln!("alpha512,{},{},{},NO_DATA", k_s, k_e, d_c);
            continue;
        }
        let percentile = |p: usize| distances[(cnt * p) / 100];
        let mean = distances.iter().sum::<usize>() as f64 / cnt as f64;
        let cprime_abs_mean_avg = cprime_abs_mean_sum / cnt as f64;
