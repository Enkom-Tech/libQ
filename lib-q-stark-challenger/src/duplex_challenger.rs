...
mod tests {
    ...
    // Goldilocks field not integrated - tests commented out
    // type G = Goldilocks;
    ...
    fn test_duplex_challenger_sample_bits_security_small_field() {
        type Mersenne31Chal = DuplexChallenger<BB, TestPermutation, WIDTH, RATE>;
        let permutation = TestPermutation {};
        let mut duplex_challenger = Mersenne31Chal::new(permutation);
