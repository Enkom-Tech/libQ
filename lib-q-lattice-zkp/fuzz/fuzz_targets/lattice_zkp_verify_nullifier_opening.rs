...
libfuzzer_sys::fuzz_target!(|data: &[u8]| {
    ...
    let proof = NullifierOpeningProof {
        nullifier,
    ...
    let ctx = data;
    let realm: &[u8] = if ctx.is_empty() {
        b"fuzz-realm"
    } else {
        &ctx[..ctx.len().min(16)]
    };
    let base_ctx = b"nullifier-fuzz";
    let _ = verify_nullifier_opening(&key, &com, realm, base_ctx, &proof, tau, z_bound);
});
