...
| Performance regression testing | **Implemented** | Criterion benches [lib-q-zkp/benches/stark_arithmetic_bench.rs](../lib-q-zkp/benches/stark_arithmetic_bench.rs) and [lib-q-zkp/benches/zkp_benchmarks.rs](../lib-q-zkp/benches/zkp_benchmarks.rs). Non-PR CI runs shard **`lib-q-zkp`** in the `performance-benches` matrix (`zkp,std`, both bench targets). PRs validate the manifest via `bench-shards-validate` and verify matrix JSON via `bench-matrix` (no Criterion runs). |
