...
| Other affected crates | Full package under test when a crate path appears in the PR diff | ≥80% line (policy); gates ratchet toward that | PR `test-coverage`: **65%** line for other affected packages; **lib-q-ml-dsa** uses **41%** for the portable (non-`simd256`) tarpaulin gate—see `pr.yml` and `coverage.yml` |
