...
| **`lib-q-threshold-sig`** | **WITHDRAWN — cryptographically unsound, provides no security; all entry points fail closed.** Not published to crates.io or npm. See [`lib-q-threshold-sig/README.md`](lib-q-threshold-sig/README.md); `lib-q-threshold-raccoon` is a candidate successor, to be evaluated on its own merits. |
...
- **Experimental / RED — not proven, not audited, pending human cryptographer sign-off (IACR review):**
  - **Unlinkable set-membership proof** in `lib-q-zkp` (Fiat–Shamir domain `libq.zkfri.membership.v0`), in two arms. **Arm A** uses value field `Complex<Mersenne31>` = GF(p²) with a degree-3 FRI challenge extension (~186 bits) and reaches 128-bit post-quantum security **only at the PCS/commitment layer** (binding on the SHAKE256 Merkle commitment); its Poseidon-over-GF(p²) round-count soundness is still unverified. **Arm B** uses the BabyBear base field with Poseidon2 and a quintic `F_{p⁵}` challenge extension (~155 bits), and likewise reaches 128-bit post-quantum security **only at the PCS/commitment layer** (binding on the SHAKE256 Merkle commitment) — upgraded from the original degree-4 config that capped at ~116-bit conjectured / ~99-bit provable; its Poseidon2 round-count soundness is still unverified. Both arms are RED and gated behind ADR-113 freeze review.
...
- **Threshold, DKG, and blind-issuance family** (`lib-q-dkg`, `lib-q-threshold-kem`, `lib-q-threshold-kem-lattice`, `lib-q-threshold-raccoon`, `lib-q-blind-token`, and related `lib-q-mac` / `lib-q-double-kem` / `lib-q-blind-pcs` / `lib-q-fhe` / `lib-q-zk-encryption-proof`): **provisional / pre-standard** post-quantum multiparty primitives, not covered by NIST standardization—see per-crate READMEs. `lib-q-threshold-sig`, formerly listed here, is **WITHDRAWN — cryptographically unsound** and excluded from this family; see [`lib-q-threshold-sig/README.md`](lib-q-threshold-sig/README.md).
