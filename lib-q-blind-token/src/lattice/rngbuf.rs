...
//!
//! The lattice samplers draw randomness in tiny chunks (a `u64` at a time for each rejection step):
//! a single keygen pulls millions of 8-byte draws. Routing each through the underlying CSPRNG's
//! per-call path dominates runtime. [`BufRng`] wraps any [`Rng`], refilling a block buffer with one
//! bulk `fill_bytes` and serving small reads from it — same byte stream, far fewer calls.
