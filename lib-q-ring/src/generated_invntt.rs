...
fn outer_3_plus<const OFFSET: usize, const STEP_BY: usize, const ZETA: i32>(
    ...
    for j in OFFSET..OFFSET + STEP_BY {
        ...
        add_coeffs(&mut re[j], &rejs);
        subtract_coeffs(&mut re[j + STEP_BY], &rej);
        montgomery_multiply_by_constant(&mut re[j + STEP_BY], ZETA);
    }
}
...
#[inline(always)]
pub fn invert_ntt_montgomery(re: &mut [Coefficients; SIMD_UNITS_IN_RING_ELEMENT]) {
...
fn invert_ntt_montgomery_finish(re: &mut [Coefficients; SIMD_UNITS_IN_RING_ELEMENT]) {
    ...
    for i in 0..re.len() {
        ...
        montgomery_multiply_by_constant(&mut re[i], 41_978);
    }
}
