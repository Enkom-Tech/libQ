...
/**
 * `@lib-q/threshold-sig` — **WITHDRAWN. This package is not a signature scheme and provides
 * no security. Every export throws; none of the shapes below can be obtained at runtime.**
 *
 * The construction was unsound at the design level, not merely pre-standard: the published
 * "public key" was the private key, and verification was a computation over public values
 * alone, so the verifier accepted signatures no participant had produced.
 *
 * If you used this package: treat any `ThresholdSigPublicKeyWire` that was ever published,
 * transmitted, logged or persisted as full disclosure of the signing key and of every party's
 * share, and rotate whatever it protected. Signatures it produced cannot be validated
 * retroactively. Any decision made on its output should be re-evaluated as unauthenticated.
 *
 * The type declarations are retained only so existing code that references them still
 * type-checks while being told, at every use site, that the scheme is withdrawn. There is no
 * drop-in replacement; choose a scheme with a stated hardness assumption and a published
 * security analysis.
 *
 * @deprecated WITHDRAWN — cryptographically unsound, provides no security. All runtime exports
 * of `@lib-q/threshold-sig` throw.
 */
export interface ThresholdSigShareVerifier {
  /**
   * **Secret material, historically published.** This field was byte-identical to the party's
   * private share. Any recorded value must be treated as disclosed.
   */
...
/** @deprecated WITHDRAWN — see {@link ThresholdSigShareVerifier}. This value was the private key. */
export interface ThresholdSigPublicKeyWire {
  profileId: number;
  threshold: number;
  /** **Secret material, historically published.** This was the master signing secret. */
  groupKeyHex: string;
  shareVerifiers: ThresholdSigShareVerifier[];
}

/** @deprecated WITHDRAWN — see {@link ThresholdSigShareVerifier}. */
...
/** @deprecated WITHDRAWN — see {@link ThresholdSigShareVerifier}. Carries no authenticity. */
...
/** @deprecated WITHDRAWN — see {@link ThresholdSigShareVerifier}. Cannot be produced at runtime. */
