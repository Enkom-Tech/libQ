...
fn gen_e<R: CryptoRng + RngCore>(e: &mut [u8; SYS_N / 8], rng: &mut R) {
    ...
    loop {
        ...
        if loop_count > MAX_LOOP_ITERATIONS {
            panic!(
                "CB-KEM gen_e failed: exceeded maximum loop iterations ({})",
                MAX_LOOP_ITERATIONS
            );
        }
...
fn gen_e<R: CryptoRng + RngCore>(e: &mut [u8], rng: &mut R) {
    ...
    loop {
        ...
        if loop_count > MAX_LOOP_ITERATIONS {
            panic!(
                "CB-KEM gen_e (8192128) failed: exceeded maximum loop iterations ({})",
                MAX_LOOP_ITERATIONS
            );
        }
...
mod tests {
    ...
    fn test_encrypt() {
        ...
        let compare_ct = TestData::new().u8vec("cbkem8192128f_encrypt_ct");
