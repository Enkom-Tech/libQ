...
mod tests {
    ...
    fn test_apply_benes() {
        let t = TestData::new();
        let mut r_arg =
            <[u8; 512]>::try_from(t.u8vec("cbkem348864_benes_apply_benes_r_before")).unwrap();
        let bits_arg =
            <[u8; 5888]>::try_from(t.u8vec("cbkem348864_benes_apply_benes_bits")).unwrap();
        apply_benes(&mut r_arg, &bits_arg, 0);
        let actual_r = r_arg;
        let expected_r =
            <[u8; 512]>::try_from(t.u8vec("cbkem348864_benes_apply_benes_r_after")).unwrap();
    ...
    fn test_apply_benes() {
        let t = TestData::new();
        let mut r_arg =
            <[u8; 1024]>::try_from(t.u8vec("cbkem460896orlarger_benes_apply_benes_r_before"))
                .unwrap();
        let bits_arg =
            <[u8; COND_BYTES]>::try_from(t.u8vec("cbkem460896orlarger_benes_apply_benes_bits"))
                .unwrap(); // TODO actual array has wrong size of 12_800
        apply_benes(&mut r_arg, &bits_arg, 0);
        let actual_r = r_arg;
        let expected_r =
            <[u8; 1024]>::try_from(t.u8vec("cbkem460896orlarger_benes_apply_benes_r_after"))
                .unwrap();
        assert_eq!(actual_r, expected_r);
    }
}
