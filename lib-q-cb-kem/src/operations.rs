...
use crate::pk_gen::pk_gen;
use crate::sk_gen::genpoly_gen;
use crate::util::{
    load_gf,
    store_gf,
};
...
pub(crate) fn crypto_kem_keypair<R: CryptoRng + Rng>(
    ...
    loop {
        ...
        if loop_count > MAX_LOOP_ITERATIONS {
            panic!(
                "CB-KEM keypair generation failed: exceeded maximum loop iterations ({})",
                MAX_LOOP_ITERATIONS
            );
        }
...
mod tests {
    ...
    fn test_crypto_kem_dec() {
        ...
        let mut sk = TestData::new().u8vec("cbkem8192128f_sk1");
        let mut c = TestData::new().u8vec("cbkem8192128f_ct1");
        let mut test_key = [0u8; 32];
        let compare_key = TestData::new().u8vec("cbkem8192128f_operations_ss");
    ...
    fn test_crypto_kem_enc() {
        ...
        let mut pk = TestData::new().u8vec("cbkem8192128f_pk1");
        assert_eq!(pk.len(), CRYPTO_PUBLICKEYBYTES);

        let compare_ss = TestData::new().u8vec("cbkem8192128f_operations_ss");
        let compare_ct = TestData::new().u8vec("cbkem8192128f_operations_enc1_ct");

        // set the same seed as in C implementation
        let entropy_input = <[u8; 48]>::try_from(
            TestData::new()
                .u8vec("cbkem8192128f_operations_entropy_input")
                .as_slice(),
        )
        .unwrap();
    ...
    fn test_crypto_kem_keypair() {
        ...
        let compare_sk = TestData::new().u8vec("cbkem8192128f_operations_sk_expected");
        assert_eq!(compare_sk.len(), CRYPTO_SECRETKEYBYTES);

        let compare_pk = TestData::new().u8vec("cbkem8192128f_operations_pk_expected");
