...
/// Encapsulation against the reference, chained onto the already-verified keygen.
///
/// The dump fixes `m` and `salt`, which are the only randomness encapsulation consumes, so the
/// ciphertext is fully determined and directly comparable. This is the half `t_4fbc9de8` left
/// uncovered: keygen agreement was the blocker, and comparing `c_kem` was meaningless until the
/// public key matched.
fn assert_encaps_matches<P: HqcParams>(set: &str) {
    let iv = dump_for(set);
    let want_ct = hex(&field(&iv, "c_kem"));
    let want_ek = hex(&field(&iv, "ek_kem"));
    let m_bytes = hex(&field(&iv, "m"));
    let salt_bytes = hex(&field(&iv, "salt"));
    assert_eq!(m_bytes.len(), 16, "{set}: reference m is not 16 bytes");
    assert_eq!(
        salt_bytes.len(),
        16,
        "{set}: reference salt is not 16 bytes"
    );
    ...
    let kem = HqcKem::<P>::new().expect("HqcKem::new");
    let (pk, sk) = kem.keygen_with_seed(&seed).expect("keygen_with_seed");

    // Guard the chaining assumption: if the public key did not match, a ciphertext mismatch below
    // would be a keygen problem wearing an encapsulation costume.
    assert_eq!(
        pk.as_bytes(),
        &want_ek[..],
        "{set}: public key does not match the reference, so the encapsulation comparison that          follows would be misattributed"
    );
    ...
    assert_eq!(
        ct.as_bytes(),
        want_ct,
        "{set}: ciphertext diverges from the reference"
    );

    // Round-trip through the verified keypair: decapsulation must recover the same shared secret.
    let recovered = kem.decapsulate(&sk, &ct).expect("decapsulate");
    assert_eq!(
        recovered.as_bytes(),
        ss.as_bytes(),
        "{set}: decapsulation of the reference ciphertext does not recover the encapsulated secret"
    );
}

#[test]
fn hqc1_encaps_matches_upstream_reference() {
    assert_encaps_matches::<Hqc1Params>("hqc-1");
}

// There is deliberately NO hqc-3 / hqc-5 encapsulation test here, and the absence is a finding
// rather than an oversight: those comparisons cannot be expressed at all against this crate's
// current API.
//
// HQC's `PARAM_SECURITY_BYTES` is 16 / 24 / 32 across HQC-128 / 192 / 256, and both the
// encapsulated message `m` and the FO rejection secret `sigma` scale with it (the reference dumps
// show exactly that; `salt` stays 16 for all three). `encapsulate_with_m_salt` takes `m: &[u8; 16]`
// for every parameter set, so a 24- or 32-byte reference `m` cannot even be passed in.
//
// That is not merely a conformance gap. The shared secret is derived from `m`, so hardcoding it at
// 16 bytes bounds the shared-secret entropy of HQC-192 and HQC-256 at 128 bits, below the level
// those parameter sets are named for. Tracked as card `t_d2ee7042`.
//
// When that card is fixed, add the two tests here. Until then, treat encapsulation for hqc-3 and
// hqc-5 as UNVERIFIED against the reference, not as verified-good: keygen matches for all three
// parameter sets, but nothing downstream of `m` has been compared for the larger two.
