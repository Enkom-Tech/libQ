...
//! deeper than the module itself:
//!   - `simd-avx2` OFF (this crate's default, and what the coverage gate measures): each
...
//!     `super::super::portable::*_portable` function.
//!   - `simd-avx2` ON: the same call sites instead execute the real AVX2 intrinsics bodies
//!     guarded by `#[cfg(all(target_arch = "x86_64", feature = "simd-avx2"))]`.
//!
//! These tests don't gate on `simd-avx2` at all, so the *same* assertions run — and must hold
//! — in both configurations: `cargo test -p lib-q-hqc` (fallback arms) and
//! `cargo test -p lib-q-hqc --features simd-avx2` (real AVX2 intrinsics). Either way they check
//! the one contract `Avx2` must uphold: bit-for-bit agreement with `Portable` on identical
//! inputs. That is real behavioural coverage of the dispatcher glue in
//! `simd/avx2/{mod,polynomial,syndrome,vector}.rs`, not a no-panic smoke test — every assertion
//! below compares a full computed output, not merely that the call returned.
...
fn test_avx2_shift_xor_matches_portable() {
    ...
    for distance in [0usize, 1, 7, 8, 63, 64, 65, 130] {
        let mut dest_avx2 = [0u64; 4];
        let mut dest_portable = [0u64; 4];
...
/// `generate_syndrome`: length not a multiple of the AVX2 32-byte chunk size, so the scalar
/// remainder path is what's actually exercised regardless of which feature set compiled it.
#[cfg(target_arch = "x86_64")]
#[test]
fn test_avx2_generate_syndrome_matches_portable() {
    let vector: [u8; 20] = core::array::from_fn(|i| (i as u8) ^ 0xDE);
    let parity: [u8; 20] = core::array::from_fn(|i| (i as u8).wrapping_mul(3) ^ 0xAD);
    let mut synd_avx2 = [0u8; 20];
    let mut synd_portable = [0u8; 20];
...
/// `correct_errors`: checks both the boolean result and the corrected buffer.
#[cfg(target_arch = "x86_64")]
#[test]
fn test_avx2_correct_errors_matches_portable() {
    let received: [u8; 16] = core::array::from_fn(|i| (i as u8).wrapping_mul(5).wrapping_add(1));
    let syndrome: [u8; 16] = core::array::from_fn(|i| (i as u8) ^ 0x2A);
    let mut corrected_avx2 = [0u8; 16];
    let mut corrected_portable = [0u8; 16];
