...
#[cfg(feature = "aes-drbg")]
#[test]
fn test_reference_behavior_exact() {
    let kat_seed = [
    ...
    println!("=== Reference Behavior Test (Two Separate 32-byte Calls) ===");

    // First call: randombytes(seed_dk, 32)
    let mut rng1 = Aes256CtrDrbg::instantiate(&kat_seed);
    let mut seed_dk = [0u8; 32];
    rng1.fill_bytes(&mut seed_dk);

    println!("First call - seed_dk: {:02x?}", seed_dk);
    println!("State after first call: {}", rng1.debug_state());

    // Second call: randombytes(seed_ek, 32) - with fresh DRBG instance
    let mut rng2 = Aes256CtrDrbg::instantiate(&kat_seed);
    let mut seed_ek = [0u8; 32];
    rng2.fill_bytes(&mut seed_ek);

    println!("Second call - seed_ek: {:02x?}", seed_ek);
    println!("State after second call: {}", rng2.debug_state());
