...
- `debug_drbg_step_by_step.rs` - Step-by-step DRBG execution
...
As of the cleanup (2025), the HQC implementation uses:
- **Pure Rust BearSSL AES** (`bearssl_aes_pure.rs`) for KAT compatibility
- **Standard Rust AES** (`aes_ctr_drbg.rs`) for general use
- **Single-backend architecture** with no C dependencies

The issues these diagnostic tests were investigating have been resolved through the BearSSL AES porting and DRBG implementation improvements.
