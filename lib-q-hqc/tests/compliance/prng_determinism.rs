...
fn test_prng_determinism_basic() {
    ...
    assert_eq!(output1, output2, "PRNG should be deterministic with same seed");
...
fn test_prng_state_independence() {
    ...
    for i in 1..outputs.len() {
        assert_eq!(outputs[0], outputs[i], "Independent PRNG instances should produce same output");
    }
...
fn test_prng_kat_seed_sequence() {
    ...
    for (i, expected_hex) in expected_seeds.iter().enumerate() {
        ...
        assert_eq!(seed, expected_bytes.as_slice(), "KAT seed {} should match expected value", i);
...
fn test_prng_byte_distribution() {
    ...
    assert!(chi_square < 400.0, "Chi-square test failed - distribution may not be uniform");
    
    // Check for obvious biases
    let min_count = byte_counts.iter().min().unwrap();
    let max_count = byte_counts.iter().max().unwrap();
    let max_deviation = (*max_count as f32 - expected_count).max(expected_count - *min_count as f32);
    ...
    assert!(max_deviation_pct < 20.0, "Maximum deviation from uniform distribution too high");
...
fn test_prng_entropy_consumption() {
    ...
    for i in 0..outputs.len() {
        for j in (i+1)..outputs.len() {
            assert_ne!(outputs[i], outputs[j], "PRNG should produce different outputs");
        }
    }
...
fn test_prng_reproducibility_across_runs() {
    ...
    for i in 1..all_outputs.len() {
        assert_eq!(all_outputs[0], all_outputs[i], "PRNG should be reproducible across runs");
    }
