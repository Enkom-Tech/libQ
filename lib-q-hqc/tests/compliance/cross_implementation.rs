...
fn test_hash_i_function() {
    ...
    assert_eq!(
        keypair_seed, keypair_seed2,
        "hash_i should be deterministic"
    );
    ...
    assert_ne!(
        keypair_seed, keypair_seed3,
        "hash_i should produce different output for different input"
    );
...
fn test_vect_sample_fixed_weight_consistency() {
    ...
    pke.vect_sample_fixed_weight1(&mut xof, &mut output1, Hqc1Params::OMEGA)
        .unwrap();
    ...
    assert_eq!(
        weight,
        Hqc1Params::OMEGA,
        "Fixed weight sampling should produce correct weight"
    );
    ...
    pke.vect_sample_fixed_weight1(&mut xof2, &mut output2, Hqc1Params::OMEGA)
        .unwrap();

    // Different seeds should produce different outputs
    assert_ne!(
        output1, output2,
        "Different seeds should produce different fixed-weight vectors"
    );
...
fn test_polynomial_multiplication_properties() {
    ...
    pke.test_vect_mul(&mut identity_result, &identity, &a)
        .unwrap();
    assert_eq!(
        identity_result, a,
        "Multiplication by identity should produce original vector"
    );
...
fn test_polynomial_addition_properties() {
    ...
    pke.test_vect_add(&mut result, &a, &b, Hqc1Params::VEC_N_SIZE_64)
        .unwrap();
    ...
    pke.test_vect_add(&mut zero_result, &zero, &a, Hqc1Params::VEC_N_SIZE_64)
        .unwrap();
    assert_eq!(
        zero_result, a,
        "Addition with zero should produce original vector"
    );

    // Test commutativity
    let mut result2 = vec![0u64; Hqc1Params::VEC_N_SIZE_64];
    pke.test_vect_add(&mut result2, &b, &a, Hqc1Params::VEC_N_SIZE_64)
        .unwrap();
...
fn test_xof_consistency() {
    ...
    assert_eq!(
        output1, output2,
        "XOF should be deterministic with same seed"
    );
    ...
    assert_ne!(
        output1, output3,
        "Different domain separators should produce different output"
    );
...
fn test_key_generation_consistency() {
    ...
    // Test that same seed produces same keypair. `keygen_with_seed` takes a 48-byte `seedKEM`
    // (the NIST `.req`/`.rsp` seed width), not a 32-byte one.
    let seed = [0u8; 48];
...
fn test_encapsulation_decapsulation_roundtrip() {
    ...
    for i in 0..10 {
        ...
        assert_eq!(
            ss1, ss2,
            "Encapsulation/decapsulation roundtrip should produce same shared secret (test {})",
            i
        );
    }
...
fn test_parameter_set_consistency() {
    ...
    for (name, n, omega, omega_r) in param_sets {
        ...
        assert!(
            omega_r > omega,
            "{}: OMEGA_R ({}) should be greater than OMEGA ({})",
            name,
            omega_r,
            omega
        );
        ...
        assert!(
            n > 10000,
            "{}: N ({}) should be large enough for security",
            name,
            n
        );
        ...
        assert!(
            omega < n / 100,
            "{}: OMEGA ({}) should be much smaller than N ({})",
            name,
            omega,
            n
        );
