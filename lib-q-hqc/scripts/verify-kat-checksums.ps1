...
$OurKats = @{
    "hqc-1" = @{
        "req" = "lib-q-hqc/kats/regression-pins/hqc-1/PQCkemKAT_2321.req"
    }
}
...
function Get-FileChecksum {
    ...
    # Hash the content with '#'-comment lines removed. Our vendored `.req` files carry a
    # provenance header comment (required by scripts/ci_guard_kat_provenance.py CHECK 5) that the
    # pristine NIST submission files do not have; hashing raw bytes would therefore report
    # "Differ" forever regardless of whether the actual seed chain matches. NIST PQCgenKAT `.req`
    # files contain no '#' lines, so stripping comments on BOTH sides compares exactly the record
    # data. (Fixed in review, lane e-kat-provenance 2026-08-05: the header addition had silently
    # turned this comparison into one that could never pass.)
    $lines = Get-Content $FilePath -Encoding UTF8 | Where-Object { -not $_.TrimStart().StartsWith('#') }
...
# Only `.req` seeds are compared; `.rsp` in `kats/regression-pins/` is self-generated by this
# crate for the Oct 2024 parameter set, not official HQC reference data.
$comparisons = @(
    @{
...
### Our KAT Files
- **Location**: `lib-q-hqc/kats/regression-pins/`
...
|----------|---------------|--------|
| `lib-q-hqc/kats/regression-pins/hqc-1/PQCkemKAT_2321.req` | `KATs/Reference_Implementation/hqc-128/hqc-128_kat.req` (under submission root) | $(if ((Compare-Files -OurFile $OurKats["hqc-1"]["req"] -OfficialFile $OfficialKats["hqc-128"]["req"] -Description "Check")) { "✅ Match" } else { "❌ Differ" }) |

## Compliance Status

$(if ($allMatch) {
"✅ **SEED CHAIN MATCH**: the compared ``.req`` seed file(s) match the official NIST HQC submission
(comment lines excluded from the comparison).

This confirms ONLY that the compared ``.req`` seed chain(s) are the NIST ones. It says nothing
about the ``.rsp`` response files, which are self-generated by this crate (see
``kats/regression-pins/PROVENANCE.md``) and are NOT compared by this script."
} else {
