...
| Regression-pin KEM KAT (all vectors) | `tests/nist_kem_kat.rs` — `hqc*_kat_count0`, `hqc*_kat_all` (pass, non-ignored) | Byte-exact `pk`/`ct`/`ss`/`sk` (NIST layout) vs `kats/regression-pins/hqc-{1,3,5}/PQCkemKAT_*_regression_pin.rsp` (self-generated by this crate, NOT official HQC reference data); SHAKE/XOF KEM flow; provenance and a known reference divergence in `kats/regression-pins/PROVENANCE.md`; CI `test-hqc` |
...
### F3 — Regression-pin KEM KAT self-consistency (resolved, bounded; NOT upstream conformance)

**Root cause:** Three separate issues blocked byte-exact agreement between this crate's own
keygen/encaps/decaps and a fixed prior run of the same code: (1) public keys were serialized as
`h ‖ s` instead of the wire format `seed_ek ‖ s`; (2) SHA3-256 domain separators for `H` and `G`
were swapped relative to the reference (`H` → domain 1, `G` → domain 0); (3) the resulting
self-generated vectors now live under `kats/regression-pins/` (NIST `.req` seeds, self-generated
`.rsp` for Oct 2024 parameters).

**Evidence:** `tests/nist_kem_kat.rs` runs non-ignored `hqc128/192/256_kat_count0` and
`hqc*_kat_all` against `kats/regression-pins/hqc-{1,3,5}/PQCkemKAT_*_regression_pin.rsp`
(self-generated by this crate — see `kats/regression-pins/PROVENANCE.md`, which also records a
length divergence from the official HQC v5.0.0 reference for HQC-192/256 `pk`/`sk`), asserting
byte-exact `pk`/`ct`/`ss`/`sk` (NIST `dk_pke ‖ sigma ‖ ek_pke`), decapsulated shared secret, and
`from_nist_bytes` round-trip. Encapsulation uses `m = seed[32..48]` and PQCgenKAT-chained
`salt`. SHA-256 pins in `kats/regression-pins/PROVENANCE.md`. This is a regression gate on this
crate's own output, not a conformance check against the official HQC reference implementation.
