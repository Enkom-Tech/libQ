//! HQC Parameters - Correct Implementation
//!
//! This module defines the correct HQC parameters based on the reference implementation.
//! HQC uses Reed-Solomon + Reed-Muller concatenated codes, not BCH codes.
...
pub trait HqcParams: Clone + fmt::Debug + PartialEq {
    ...
    /// Parameter N (polynomial degree)
    const N: usize;
    /// Parameter N1 (Reed-Solomon code length)
    const N1: usize;
    /// Parameter N2 (Reed-Muller code length)
    const N2: usize;
    /// Parameter N1N2 (concatenated code length in bits)
    const N1N2: usize;
    /// Parameter K (Reed-Solomon information bits)
    const K: usize;
    /// Parameter OMEGA (weight for x, y)
    const OMEGA: usize;
    /// Parameter OMEGA_E (weight for error e)
    const OMEGA_E: usize;
    /// Parameter OMEGA_R (weight for r1, r2)
    const OMEGA_R: usize;
    /// Reed-Solomon error correction capacity
    const DELTA: usize;
    /// Galois field degree m for GF(2^m)
    const M: usize;
    /// Generator polynomial of GF(2^M)
    const GF_POLY: u16;
    /// Size of multiplicative group of GF(2^M)
    const GF_MUL_ORDER: usize;
    /// Generator polynomial size for Reed-Solomon
    const G: usize;
    ...
    const VEC_N_SIZE_BYTES: usize;
    const VEC_N1_SIZE_BYTES: usize;
    const VEC_N1N2_SIZE_BYTES: usize;

    /// Vector sizes in 64-bit words
    const VEC_N_SIZE_64: usize;
    ...
    const UTILS_REJECTION_THRESHOLD: u32;
...
/// HQC-1 parameters (128-bit security)
#[derive(Debug, Clone, PartialEq)]
pub struct Hqc1Params;

impl HqcParams for Hqc1Params {
    ...
    const OMEGA_R: usize = 75; // Official HQC-1 parameter
    const DELTA: usize = 15; // Official HQC-1 parameter
    const M: usize = 8; // Official HQC-1 parameter
    const GF_POLY: u16 = 0x11D; // Official HQC-1 parameter
    const GF_MUL_ORDER: usize = 255; // Official HQC-1 parameter
    ...
    const ALPHA_IJ_POW: &'static [[u16; 55]; 32] = &[
        [
            2, 4, 8, 16, 32, 64, 128, 29, 58, 116, 232, 205, 135, 19, 38, 76, 152, 45, 90, 180,
            117, 234, 201, 143, 3, 6, 12, 24, 48, 96, 192, 157, 39, 78, 156, 37, 74, 148, 53, 106,
            212, 181, 119, 238, 193, 159, 35, 70, 140, 5, 10, 20, 40, 80, 160,
        ],
        [
            4, 16, 64, 29, 116, 205, 19, 76, 45, 180, 234, 143, 6, 24, 96, 157, 78, 37, 148, 106,
            181, 238, 159, 70, 5, 20, 80, 93, 105, 185, 222, 95, 97, 153, 94, 101, 137, 30, 120,
            253, 211, 107, 177, 254, 223, 91, 113, 217, 67, 17, 68, 13, 52, 208, 103,
        ],
        [
            8, 64, 58, 205, 38, 45, 117, 143, 12, 96, 39, 37, 53, 181, 193, 70, 10, 80, 186, 185,
            161, 97, 47, 101, 15, 120, 231, 107, 127, 223, 182, 217, 134, 68, 26, 208, 206, 62,
        ...
        [
            58, 45, 12, 37, 193, 80, 161, 101, 231, 223, 134, 208, 237, 102, 169, 168, 146, 191,
            179, 150, 87, 7, 166, 195, 36, 251, 125, 173, 64, 38, 143, 39, 181, 10, 185, 47, 120,
            59, 85, 150, 89, 44, 38, 193, 15, 26, 169, 145, 100, 36, 1, 96, 185, 223, 59, 85, 150,
            89, 44, 38, 193, 15, 26, 169, 145, 100, 36, 1, 96, 185, 223, 59,
        ],
        [
            192, 222, 182, 151, 114, 110, 155, 27, 143, 160, 177, 237, 82, 75, 89, 88, 152, 70,
            240, 103, 21, 123, 224, 251, 116, 212, 101, 136, 218, 145, 200, 144, 8, 78, 190, 217,
            204, 183, 87, 172, 216, 12, 105, 225, 59, 170, 98, 242, 250, 180, 10, 211, 31, 168,
            255,
        ],
        [
            157, 95, 217, 133, 230, 130, 18, 2, 39, 190, 175, 23, 209, 25, 36, 4, 78, 97, 67, 46,
            191, 50, 72, 8, 156, 194, 134, 92, 99, 100, 144, 16, 37, 153, 17, 184, 198, 200, 61,
            32, 74, 47, 34, 109, 145, 141, 122, 64, 148, 94, 68, 218, 63, 7, 244,
        ],
    ];
}

/// HQC-3 parameters (192-bit security)
#[derive(Debug, Clone, PartialEq)]
pub struct Hqc3Params;

impl HqcParams for Hqc3Params {
    ...
    const VEC_N_SIZE_BYTES: usize = 4482; // CEIL_DIVIDE(35851, 8)
    const VEC_N1_SIZE_BYTES: usize = 56; // PARAM_N1 directly
    const VEC_N1N2_SIZE_BYTES: usize = 4480; // CEIL_DIVIDE(35840, 8)

    const VEC_N_SIZE_64: usize = 561; // CEIL_DIVIDE(35851, 64)
...
/// HQC-5 parameters (256-bit security)
#[derive(Debug, Clone, PartialEq)]
pub struct Hqc5Params;

impl HqcParams for Hqc5Params {
    ...
    const VEC_N_SIZE_BYTES: usize = 7205; // CEIL_DIVIDE(57637, 8)
    const VEC_N1_SIZE_BYTES: usize = 90; // PARAM_N1 directly
    const VEC_N1N2_SIZE_BYTES: usize = 7200; // CEIL_DIVIDE(57600, 8)
...
/// Helper function to create bitmask
#[inline]
pub const fn bitmask(a: usize, size: usize) -> usize {
    (1usize << (a % size)) - 1
}
...
mod tests {
    ...
    fn test_hqc3_parameters() {
        assert_eq!(Hqc3Params::N, 35851);
        assert_eq!(Hqc3Params::N1, 56);
    ...
    fn test_hqc5_parameters() {
        assert_eq!(Hqc5Params::N, 57637);
        assert_eq!(Hqc5Params::N1, 90);
    ...
    fn test_vector_sizes() {
        // Test that vector sizes are correctly computed
        assert_eq!(Hqc1Params::VEC_N_SIZE_BYTES, ceil_divide(Hqc1Params::N, 8));
        assert_eq!(Hqc1Params::VEC_N1_SIZE_BYTES, 46);
        assert_eq!(
            Hqc1Params::VEC_N1N2_SIZE_BYTES,
            ceil_divide(Hqc1Params::N1N2, 8)
        );

        assert_eq!(Hqc1Params::VEC_N_SIZE_64, ceil_divide(Hqc1Params::N, 64));
        assert_eq!(Hqc1Params::VEC_N1_SIZE_64, ceil_divide(Hqc1Params::N1, 8));
        assert_eq!(
    ...
    #[test]
    fn test_reed_solomon_coefficients() {
        // Test that Reed-Solomon coefficients are provided
        assert_eq!(Hqc1Params::RS_POLY_COEFS.len(), Hqc1Params::G);
