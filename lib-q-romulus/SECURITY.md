...
| `SaturninSiv` (`lib-q-aead`, feature `saturnin-siv`) | 256 / 256-bit | no key-commitment claim made; the *generic* CMT-1 cost is ~2^128 from the 256-bit tag alone. **The reason this mode exists is misuse resistance, not commitment**: it is the only AEAD here that is both nonce-misuse-resistant AND 256-bit. Romulus-M is MRAE but 128-bit (~2^64 under Grover); Saturnin's other modes are 256-bit but nonce-respecting. Saturnin-CTR + KMAC256 in the Rogaway-Shrimpton SIV arrangement, assembled in-repo. Deterministic, so nonce reuse is non-catastrophic but identical `(nonce, AD, plaintext)` tuples are linkable. Two-pass, non-streaming. NOT RFC 5297-interoperable: canonical AES-SIV feeds the tag straight in as the CTR IV, which would funnel a 256-bit tag through Saturnin-CTR's 16-byte nonce and cap keystream separation at a ~2^64 birthday — the exact weakness this mode exists to remove — so the keystream key is instead derived by a PRF over the whole tag. **RED**: no cryptographer has reviewed this instantiation, its MRAE property is inherited from Rogaway-Shrimpton given that KMAC256 is a PRF and Saturnin-CTR is a secure stream cipher (neither premise established here), and its KATs are self-generated so there is no external conformance evidence. |
