...
/// Public key seed for SLH-DSA
#[derive(Clone, Debug, PartialEq, Eq)]
pub struct PkSeed<N: ArraySize>(pub(crate) Array<u8, N>);
...
impl<P: ParameterSet + VerifyingKeyLen> VerifyingKey<P> {
    ...
    fn raw_slh_verify_internal(
        ...
        let digest = hasher.h_msg(randomizer, &self.pk_root, msg);
        ...
        P::ht_verify(&hasher, &fors_pk, ht_sig, idx_tree, idx_leaf, &self.pk_root)
            .then_some(())
            .ok_or(Error::new())
    }
    ...
    fn raw_try_verify_with_context(
        ...
        self.raw_slh_verify_internal(&ctx_msg, signature)
    }
...
impl<P: ParameterSet> MultipartVerifier<Signature<P>> for VerifyingKey<P> {
    fn multipart_verify(&self, msg: &[&[u8]], signature: &Signature<P>) -> Result<(), Error> {
        self.raw_try_verify_with_context(msg, &[], signature)
    }
}
