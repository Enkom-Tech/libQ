...
**From:** GIP-side perf pass, 2026-06-20. **To:** whoever owns lib-q performance.
...
- `gip-crypto` `SaturninEngine`/`RomulusM1Engine` now call the slice methods → the two per-packet
...
The GIP hot path is fully covered by the two engines above; this last item is **libQ-internal polish,
not on any GIP critical path** — handed over so it lives with the trait owner, not GIP:
...
3. **No new vectors needed** — this is a signature/forwarding refactor; the existing AEAD KATs + the
   gip-crypto `*_round_trip_via_record_api` tests already pin byte-for-byte behavior. Just keep them green.

Reason it's a *cleanup* and not a fix: the only two AEADs GIP actually wraps per-packet (Saturnin
default, Romulus interop) are already alloc-free. Promoting to the trait removes the asymmetry and
prevents a future AEAD from silently regressing back to wrapper-allocs, but buys no measurable GIP win
...
Separate, much simpler win — surfaced while auditing the GIP data-plane seal/open hot path.

**`lib-q-core`'s `Aead` trait takes `&AeadKey` / `&Nonce` (`traits.rs:249,268`), both `Vec<u8>`-backed
under `alloc`.** So every record seal *and* open on the GIP side does two throwaway heap allocations
just to wrap fixed-size arrays into those structs — e.g. `gip-crypto`'s `SaturninEngine::encrypt`:
...
GIP can't fix this from its side without either this trait change or threading cached `AeadKey`
...
GIP-proper data-plane shape is otherwise already optimal — the sender datagram builder pre-sizes the
whole buffer (`Vec::with_capacity`) and appends the sealed record in place with no intermediate
record buffer; the relay forward path was reduced to a single alloc this same pass (GIP `master`
