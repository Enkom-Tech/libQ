...
//! samplers⇒folds), validated separately by `crate::compose`'s vertical slice; wiring it into this
//! same batch (sharing the fold instances) closes the full proof. `ζ` is verifier-recomputed from the
//! ciphertext — never prover-supplied.
//!
