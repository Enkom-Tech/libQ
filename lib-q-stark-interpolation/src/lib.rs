...
mod tests {
    ...
    fn test_interpolate_subgroup_degree_3_correctness() {
        ...
        type EF4 = BinomialExtensionField<Complex<Mersenne31>, 2>;
        ...
        // Convert base field to extension field through intermediate Complex extension
        let poly = |x: EF4| {
            let two = EF4::from(Complex::from(F::TWO));
            let three = EF4::from(Complex::from(F::from_u32(3)));
            let four = EF4::from(Complex::from(F::from_u32(4)));
            x * x * x + x * x * two + x * three + four
        };
    ...
    fn test_interpolate_coset_multiple_polynomials() {
        ...
        let f2 = |x: EF4| {
            let four = EF4::from(Complex::from(F::from_u32(4)));
            let five = EF4::from(Complex::from(F::from_u32(5)));
            let six = EF4::from(Complex::from(F::from_u32(6)));
            x * x * four + x * five + six
        };
