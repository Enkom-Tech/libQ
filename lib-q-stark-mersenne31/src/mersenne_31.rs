...
use lib_q_stark_field::exponentiation::exp_1717986917;
use lib_q_stark_field::integers::QuotientMap;
use lib_q_stark_field::op_assign_macros::{
    impl_add_assign,
    impl_div_methods,
    impl_mul_methods,
    impl_sub_assign,
...
/// Constant-time equality comparison for Mersenne31.
///
/// This implementation uses constant-time comparison to prevent timing attacks
/// when comparing secret field elements.
impl ConstantTimeEq for Mersenne31 {
    fn ct_eq(&self, other: &Self) -> subtle::Choice {
        // Compare canonical representations in constant time.
        // The raw `value` field may hold either 0 or P (== 2^31 − 1) for
        // the additive identity; using as_canonical_u32() normalises both to
        // 0 before comparison so that P ct_eq 0 returns true, as required.
        self.as_canonical_u32().ct_eq(&other.as_canonical_u32())
    }
}
...
impl TwoAdicField for Mersenne31 {
    /// TWO_ADICITY = 1 for the base field.
    ///
    /// For p = 2^31 − 1, the multiplicative group has order p − 1 = 2^31 − 2
    /// = 2 * (2^30 − 1). Since 2^30 − 1 is odd, exactly one factor of 2
    /// divides p − 1, so TWO_ADICITY = 1.
    ///
    /// Note: `Complex<Mersenne31>` (the degree-2 extension) has TWO_ADICITY = 32;
    /// that applies to the extension field, not this base field.
    ...
    fn two_adic_generator(bits: usize) -> Self {
        ...
        if bits == 0 {
            ...
            // The unique subgroup of order 2 is generated by −1.
            Self::NEG_ONE
        }
    }
}
