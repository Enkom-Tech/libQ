...
impl ConstantTimeEq for Mersenne31 {
    fn ct_eq(&self, other: &Self) -> subtle::Choice {
        ...
        // Both values are already in range [0, 2^31-1], so we can compare directly
        // Using u32 comparison which is constant-time on most architectures
        subtle::Choice::from((self.value == other.value) as u8)
    }
}
...
impl TwoAdicField for Mersenne31 {
    /// The Mersenne31 field has TWO_ADICITY = 31
    /// because the multiplicative group has order (2^31 - 1) - 1 = 2^31 - 2 = 2 * (2^30 - 1)
    /// Wait, that's wrong. Let me recalculate:
    /// For p = 2^31 - 1, the multiplicative group has order p - 1 = 2^31 - 2 = 2 * (2^30 - 1)
    /// So TWO_ADICITY = 1? No, that doesn't match the complex extension which says 31.
    ///
    ...
    fn two_adic_generator(bits: usize) -> Self {
        ...
        if bits == 0 {
            ...
            // For TWO_ADICITY = 1, the generator of order 2 is -1
            Self::NEG_ONE
        }
    }
}
