...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    if let Ok(kem_aead) = create_aead(Algorithm::KemAead) {
        let start = Instant::now();
        for _ in 0..iterations {
            let _ = kem_aead.encrypt(&key, &nonce, &plaintext, Some(associated_data))?;
        }
        let kem_time = start.elapsed();

        println!("KEM AEAD: {:?} avg", kem_time / iterations as u32);
    }
